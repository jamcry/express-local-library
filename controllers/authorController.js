...
exports.author_create_post = [
  ...
  body('data_of_birth', 'Invalid date of birth').optional({ checkFalsy: true }).isISO8601(),
...
exports.author_delete_get = function(req, res) {
  ...
  async.parallel({
    ...
    if(err) return next(err);
    // Redirect to author catalog if author to be deleted does not exist
    if(results.author==null) {
...
exports.author_delete_post = function(req, res) {
  ...
  async.parallel({
    ...
    authors_books: function(calllback) {
      Book.find({ 'author': req.body.authorid }).exec(calllback)
    }
  }, function(err, results) {
    if(err) return next(err);
